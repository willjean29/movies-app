...
export const registerYuupSchema = yupObject().shape({
  ...
  [RegisterFieldName.Email]: yupString().required(ErrorMessage.Required).matches(Regex.email, {
    message: ErrorMessage.InvalidEmail,
    excludeEmptyString: true,
  }),
  [RegisterFieldName.Password]: yupString().required(ErrorMessage.Required).matches(Regex.password, {
